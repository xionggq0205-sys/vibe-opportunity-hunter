...
默认走"两层输出"——上来不砸你 200 行报告，先给精简卡片：

**精简层（默认）**：先看 TL;DR 排序表，然后每个机会 5-6 行：
- 一条最强引用（带 URL）
- 痛点 / 用户 / MVP / 验证动作 各一行

**深挖层（按需）**：想看某个机会的完整研究就说"深挖 1 号"或"看 N 号细节"，会展开：
- 全部 2-3 条引用
- 25★ 评分 5 维度细分（频次/新鲜度/强度/用户清晰度/替代方案空白）
- MVP 完整建议 + 48h 验证 step-by-step
- 竞品现状 + 已知风险

幕后调研严谨度跟深挖层一样（真实 URL、跨平台、5 维度评分），只是展示时默认收起。结尾会有主动跟进钩子告诉你这 4 个选项。
